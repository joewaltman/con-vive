...
import { getTrojanDinner, getTrojanNextWaitlistPosition } from "@/lib/trojan-dinner";
import { sendEmail } from "@/lib/email";
import TrojanWaitlistConfirmationEmail from "@/emails/trojan-waitlist-confirmation";
...
export async function POST(request: Request) {
  try {
    ...
    const position = await getTrojanNextWaitlistPosition(dinner.id);
    ...
    console.log(
      `[Trojan Waitlist] Added guest ${guestId} at position ${actualPosition}`
    );
    ...
    try {
      ...
      await sendEmail({
        ...
        subject: "You're on the waitlist for the Trojan Alumni dinner",
        react: TrojanWaitlistConfirmationEmail({
    ...
    console.error("Trojan waitlist error:", error);
    return NextResponse.json(
      { error: "Failed to join waitlist" },
      { status: 500 }
    );
