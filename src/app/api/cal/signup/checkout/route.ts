...
export async function POST(request: Request) {
  try {
    ...
    if (invitation.stripe_checkout_session_id) {
      try {
        ...
        if (existingSession.status === "open" && hasTimeLeft) {
          console.log(`[Cal Checkout] Reusing existing session ${existingSession.id}`);
        ...
        if (existingSession.status === "complete" && existingSession.payment_status === "paid") {
          console.log(`[Cal Checkout] Found completed session, reconciling invitation ${invitation.id}`);
          await query(
            `UPDATE invitations
             SET status = 'booked',
                 booked_at = NOW(),
        ...
        console.log(`[Cal Checkout] Could not retrieve existing session: ${error}`);
      }
    }
    ...
    console.log(`[Cal Checkout] Created new session ${session.id} for invitation ${invitation.id}`);
