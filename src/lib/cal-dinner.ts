...
export async function getCalDinner(): Promise<CalDinnerRow | null> {
  const dinnerId = process.env.CAL_DINNER_ID;
  if (!dinnerId) {
    console.error("CAL_DINNER_ID environment variable is not set");
    return null;
  }

  const result = await query<CalDinnerRow>(
    `SELECT
      ...
      COALESCE(total_seats, 7) as total_seats,
      COALESCE(signup_source, 'con-vive') as signup_source,
      COALESCE(enforce_gender_balance, true) as enforce_gender_balance,
      bring_items
    FROM dinners
    WHERE id = $1`,
...
export async function getConfirmedSeatCount(dinnerId: number): Promise<number> {
...
export async function getRemainingSeats(dinnerId: number): Promise<number> {
  const result = await query<{ remaining: string }>(
    `SELECT
      GREATEST(0, COALESCE(d.total_seats, 7) - COUNT(i.id)) as remaining
     FROM dinners d
...
export async function isDinnerFull(dinnerId: number): Promise<boolean> {
  // Uses SELECT FOR UPDATE to prevent race conditions during checkout
  const result = await query<{ is_full: boolean }>(
    `SELECT
      (COUNT(i.id) >= COALESCE(d.total_seats, 7)) as is_full
     FROM dinners d
     LEFT JOIN invitations i ON i.dinner_id = d.id AND i.status = 'booked'
     WHERE d.id = $1
     GROUP BY d.id, d.total_seats
...
export async function getNextWaitlistPosition(dinnerId: number): Promise<number> {
...
export async function getCalAlumniGuests(dinnerId: number): Promise<GuestForExport[]> {
...
export async function getWaitlistGuests(dinnerId: number): Promise<Array<GuestForExport & { position: number }>> {
...
export function formatAlumniForExport(guests: GuestForExport[]): string {
  return guests
    .map((g) => {
