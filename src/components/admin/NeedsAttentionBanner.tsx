...
export default function NeedsAttentionBanner({ guestId, guestName, onActionComplete }: NeedsAttentionBannerProps) {
  ...
  useEffect(() => {
    async function checkStatus() {
      try {
        const response = await fetch(`/api/admin/guests/${guestId}/needs-attention`);
        if (response.ok) {
  ...
  const handleApprove = useCallback(async (message?: string) => {
    ...
    try {
      console.log('[NeedsAttentionBanner] Fetching /api/admin/guests/${guestId}/approve...');
      const response = await fetch(`/api/admin/guests/${guestId}/approve`, {
        method: 'PATCH',
  ...
  const handleArchive = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/admin/guests/${guestId}/archive`, {
        method: 'POST',
      });
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/admin/guests/${guestId}/reject`, {
        method: 'POST',
      });
