...
export default function DinnerDetail({ dinnerId }: DinnerDetailProps) {
  ...
  const { data: dinner, error, isLoading } = useSWR<Dinner>(
    `/api/admin/dinners/${dinnerId}`,
    fetcher,
    { revalidateOnFocus: false }
  );

  // Fetch hosts for dropdown
  const { data: hosts } = useSWR<Host[]>('/api/admin/hosts', fetcher, {
    revalidateOnFocus: false,
  });
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const response = await fetch(`/api/admin/dinners/${dinnerId}`, {
        method: 'PATCH',
      ...
      mutate(`/api/admin/dinners/${dinnerId}`);
  ...
  const handleInvitationResponseChange = useCallback(async (
    ...
    setActionError(null);
    try {
      const res = await fetch(`/api/admin/invitations/${invitationId}/response`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleMarkDeclined = useCallback(async (invitationId: number) => {
    setActionError(null);
    try {
      const res = await fetch(`/api/admin/invitations/${invitationId}/mark-declined`, {
        method: 'POST',
      });
  ...
  const handleAddBringItem = useCallback(async (
    ...
    const response = await fetch(`/api/admin/dinners/${dinnerId}/bring-items`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteBringItem = useCallback(async (itemId: number) => {
    const response = await fetch(`/api/admin/bring-items/${itemId}`, {
      method: 'DELETE',
    });
      ...
      <div className="bg-white border border-gray-200 rounded-lg">
        ...
        <div>
          {!dinner.invitations || dinner.invitations.length === 0 ? (
            ...
            [...dinner.invitations]
              .sort((a, b) => {
                // Put booked (paid) guests at the top
