...
interface CalSignupFormProps {
  bringItems: BringItem[];
  isWaitlistMode: boolean;
  onWaitlistSuccess: () => void;
}

export function CalSignupForm({
  bringItems,
  isWaitlistMode,
  onWaitlistSuccess,
}: CalSignupFormProps) {
  ...
  async function handlePage1Submit() {
    ...
    try {
      if (isWaitlistMode) {
        // Submit to waitlist
        const res = await fetch("/api/cal/waitlist", {
          method: "POST",
      ...
      const res = await fetch("/api/cal/signup/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          firstName: formData.firstName,
  ...
  async function handlePage2Submit() {
    ...
    try {
      ...
      const completeRes = await fetch("/api/cal/signup/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          guestId,
      ...
      const checkoutRes = await fetch("/api/cal/signup/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          guestId,
